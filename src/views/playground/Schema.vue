          ...
          <div class="container">
            ...
            <div class="form-group">
              ...
              <label for="schDescription"
                ><strong
                  >Description<span style="color: red">*</span>:</strong
                ></label
              >
              ...
              <textarea
                ...
                placeholder="Enter Description for this schema"
              ></textarea>
            </div>
            <div class="form-group card">
              <b-card-header
                header-tag="header"
                class="p-1 border-0 accordin-header theme-color"
                role="tab"
              >
                <b-button
                  block
                  ...
                  title="Create schema configuration"
                  >Fields Configurations
                  <i
                    :class="!visible ? 'fa fa-arrow-down' : 'fa fa-arrow-up'"
                    style="float: right"
                  ></i>
                </b-button>
              </b-card-header>
              ...
              >
                ...
                <div class="row g-3 align-items-center w-100">
                  ...
                  <div class="col-lg-9 col-md-9 px-0">
                    <input
                      v-model="selected.attributeName"
                      type="text"
                      id="attributeName"
                ...
                <div class="row g-3 align-items-center w-100 mt-4">
                  ...
                  <div class="col-lg-9 col-md-9 px-0">
                    <input
                      type="checkbox"
                      v-model="selected.attributeRequired"
                      id="required"
                    />
                  </div>
                </div>
        ...
        >
          ...
          <tbody>
            <tr v-for="row in schemaList" :key="row._id">
              <td>
                <div v-if="row.schemaId">
                  <a
                    ...
                    target="_blank"
                    >{{ row.schemaId ? shorten(row.schemaId) : "-" }}</a
                  >
                  <i
              ...
              <td>
                {{ row.schemaDetails ? row.schemaDetails.modelVersion : "-" }}
              </td>
              <td class="word-wrap">
                {{
                  row.schemaDetails ? row.schemaDetails.schema.description : "-"
                }}
              </td>
              <td>
                <div v-if="row.schemaDetails">
                  <div
                    v-for="prop in Object.keys(
                      row.schemaDetails.schema.properties
                    )"
                    style="display: inline-block"
                  >
                    <b-badge pill variant="info" class="mr-2">{{
                      prop
                    }}</b-badge>
                  </div>
                </div>
  ...
  methods: {
    ...
    updateSchemaAttribute() {
      ...
      if (isValid) {
        ...
        const indexToUpdate = this.attributes.findIndex(
    ...
    handleValidation(update = false) {
      ...
      if (isEmpty(this.selected.attributeName)) {
        ...
        return this.notifyErr(
          this.selected.attributeName + " " + message.SCHEMA.PROTECTED_TERM
        );
    ...
    isPresent(attr, update = false) {
      const element = this.attributes.find((x) => {
        if (update) {
          return (
            x.name === attr.attributeName &&
            x.type === attr.attributeTypes &&
            x.isRequired === attr.attributeRequired
          );
        }
    ...
    ssePopulateSchema(id, store) {
      const sse = new EventSource(
        `${this.$config.studioServer.SCHEMA_SSE}${id}`
      );
    ...
    createSchema() {
      try {
        ...
        } else if (!isPascalCase(this.credentialName)) {
          return this.notifyErr(message.SCHEMA.NOT_PASCALCASE_SCHEMA_NAME);
        ...
        fetch(url, {
          ...
          .then((j) => {
            ...
            if (j.message === "success") {
              this.notifySuccess(
                "Schema creation initiated. Please approve the transaction from your wallet"
              );
