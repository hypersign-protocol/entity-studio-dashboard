                ...
                <form style="padding: 5px">
                  ...
                  <div v-else>
                    <div class="form-group">
                      <tool-tip
                        infoMessage="Credential issued to this DID"
                      ></tool-tip>
                    ...
                    <div class="form-group">
                      <tool-tip
                        infoMessage="Enter Issuer DID for this Credential"
                      ></tool-tip>
                  ...
                  <div class="form-group pt-2" v-if="isEdit === true">
                    <tool-tip
                      infoMessage="Issuance Date of the issued credential"
                    ></tool-tip>
                  ...
                  <div class="form-group pt-2" v-if="isEdit === true">
                    <tool-tip
                      infoMessage="Expiry Date for the issued credential"
                    ></tool-tip>
                  ...
                  <div class="form-group" v-if="isEdit === true">
                    <tool-tip
                      infoMessage="Select Status for the issued credential"
                    ></tool-tip>
                    <label for="forselectschema"
                      ><strong
          ...
          <tbody>
            <tr v-for="row in vcList" :key="row._id">
              <td>
                <div v-if="row.vc">
                  <a
                    ...
                    }revocationRegistry/${removeUrl(row.vc.id)}`"
                    target="_blank>"
                    >{{ row.vc.id ? shorten(row.vc.id) : "-" }}</a
                  >
                  <i
              ...
              <td>
                <div style="display: flex">
                  <a
                    ...
                    target="_blank"
                    >{{ row.vc ? row.vc.type[row.vc.type.length - 1] : "-" }}
                  </a>
                  <i
                    class="far fa-copy ml-1"
                    v-if="row.vc"
              ...
              <td>
                <div style="display: flex">
                  <a
                    :href="`${$config.explorer.BASE_URL}identity/${row.subjectDid}`"
                    target="_blank"
                    >{{ shorten(row.subjectDid) }}
                  </a>
                  <i
                    class="far fa-copy ml-1"
                    style="cursor: pointer"
    ...
    async OnSchemaSelectDropDownChange(event) {
      try {
        ...
        if (event) {
          ...
          this.isLoading = true;
          let selectedSchemas = await this.findOrFetchSchemaBySchemaID(event);
          ...
          this.issueCredentialType = selectedSchemas.schemaDetails.name;
